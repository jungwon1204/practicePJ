...
def first_generation(num):
    ...
    print('random_country_first :',random_country)
    
    return random_country

#적합도 함수
def sum_value(num):
    print(f"def_4 : {test_2}")
...
#적합도 함수_2
def sum_value_2(num):
    print(f"def_5(적합도 안) : {test_2}")
    score_list = []
    print(f"적합도 안 test : {test}, test_2 :{test_2}")
    
    print('적합도 안 random_country_last \n', random_country)
...
def play(times):
    ...
    for i in range(times):
        ...
        ##print(i+1, '번')
        print(f"print_last random_country{random_country}")
        test = copy.deepcopy(random_country)
        global test_2
        test_2 = 3
        print(f"test:{test}, test_2 {test_2}")
        
        sum_value_2(GA_TSP) #세대 적합도 구하는 함수 (score_list 값 반환)
